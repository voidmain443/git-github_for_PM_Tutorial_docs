...
module.exports=async(context,base,check,root)=>{
 ...
 try{
  await page.goto(base+'learn.html?unit=0');await ready();
  ...
  const chapter=await details('study-chapter'),chapterText=await chapter.innerText();
  check('whole chapter includes all available steps',initial.guideSteps.filter(s=>!s.locked).every(s=>chapterText.includes(s.title)));
  check('whole chapter is more than the selected step',chapterText.length>(await page.locator('#step-body').innerText()).length);
  ...
  for(const value of [8,9]){
   ...
   const text=await (await details('study-chapter')).innerText();
  ...
  for(let value=0;value<10;value++){
   ...
   const text=await (await details('study-chapter')).innerText();
   check('unit '+value+' has full chapter reading',all[value].guideSteps.every(s=>text.includes(s.title)));
   await details('study-simulation');const frame=await frameReady();
  ...
  await details('study-chapter');await details('study-simulation');
  const mobileFrame=await frameReady();
  check('complete chapter and simulation fit mobile',await fits());

...
function prompt(title,items,file){return `<section class="wb-prompt"><h3>${title}</h3><ol>${items.map(s=>'<li>'+s+'</li>').join('')}</ol>${file?`<a href="learn.html?unit=${modules[active].unit}">교재의 웹 양식에서 작성하기 →</a> · <a href="forms/${encodeURIComponent(file.replace(/\.md$/,''))}.html" target="_blank" rel="noopener">빈 양식 읽기 ↗</a> · <a download href="templates/${encodeURIComponent(file)}">${esc(file.replace('.md',''))} 양식 내려받기 →</a>`:''}</section>`;}
...
async function workflow(){
 ...
 const motion=playback(document.querySelector('#flow-detail').parentElement,notes.map(n=>n[0]+' · '+n[1]),choose,{diagram:'#flow-diagram'});
 document.querySelectorAll('[data-flow]').forEach(b=>b.onclick=()=>motion.select(Number(b.dataset.flow)));return motion;
}
async function charter(){
 ...
 const motion=playback(document.querySelector('#charter-detail').parentElement,steps.map(s=>s[0]+' · '+s[1]+' · '+s[2]),choose,{diagram:'#charter-diagram'});
 document.querySelectorAll('[data-charter]').forEach(b=>b.onclick=()=>motion.select(Number(b.dataset.charter)));return motion;
}
...
async function scheduleLab(){
 ...
 let revealed=false,motion=null;
 const calculate=()=>{
  motion?.();motion=null;
  ...
  motion=schedulePlayback(model,result,lag);
 };
...
async function change(){
 ...
 const barRows=document.querySelectorAll('.wb-bar-row');
 const host=document.createElement('div');document.querySelector('.wb-bars').after(host);
 return playback(host,['PV: 기준일까지 계획한 작업의 예산 가치입니다. 기준선이 분모를 제공합니다.','EV: 실제 완료한 작업을 계획된 예산 가치로 환산합니다. 지급액이나 개발자의 체감 진척이 아닙니다.','AC: 같은 작업·같은 기준일까지 인식한 실제 비용입니다. 현금 지급과 미지급을 다시 합산하지 않습니다.','EV − PV는 일정 가치 편차, EV − AC는 원가 편차입니다. SPI를 지연 일수로 바꾸려면 추가 일정 분석이 필요합니다.'],i=>{window.d3.selectAll(barRows).interrupt().transition().duration(matchMedia('(prefers-reduced-motion: reduce)').matches?0:400).style('opacity',(_,j)=>i===3||i===j?1:.35);});
...
async function load(){
 ...
 try{
  ...
  const m=modules[active];history.replaceState(null,'','visual.html?lab='+active+(embedded?'&embed=1':''));
...
$('#wb-download').onclick=()=>{
 ...
 content.querySelectorAll('button,.wb-slider-grid,.wb-selectors,.wb-reading-links,.wb-playback .wb-controls,.extra-player,.extra-check,.wb-slider').forEach(e=>e.remove());
